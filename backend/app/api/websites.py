...
@router.post("/{website_id}/php")
async def update_website_php(website_id: int, php_version: str, db: Session = Depends(get_db)):
    website = db.query(models.Website).filter(models.Website.id == website_id).first()
    if not website:
        raise HTTPException(status_code=404, detail="Website not found")
    
    success = WebsiteService.change_php_version(website.domain, php_version)
    if not success:
         raise HTTPException(status_code=500, detail="Failed to update PHP configuration")
         
    website.php_version = php_version
    db.commit()
    return {"status": "success", "message": f"PHP version changed to {php_version}"}

@router.post("/{website_id}/ssl")
async def toggle_website_ssl(website_id: int, db: Session = Depends(get_db)):
    website = db.query(models.Website).filter(models.Website.id == website_id).first()
    if not website:
        raise HTTPException(status_code=404, detail="Website not found")
    ...
    db.commit()
    return {"status": "success", "ssl_enabled": website.ssl_enabled}
